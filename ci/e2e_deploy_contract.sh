...
node index.js DANGER deploy_test_erc20 > /tmp/eth2tezostransfer.out
cat /tmp/eth2tezostransfer.out | xargs node index.js deploy-token myerc20 $1

node index.js transfer-eth-erc20-to-tezos --token-name myerc20 --amount 1000 \
...
node index.js transfer-eth-erc20-from-tezos --token-name myerc20 --amount 1 --tezos-sender-account rainbow_bridge_eth_on_tezos_prover \
