...
const { borshify, borshifyInitialValidators } = require('../tezbridge/borsh')

/// Maximum number of retries a Web3 method call will perform.
const MAX_WEB3_RETRIES = 1000

class Tezos2EthRelay {
  async initialize() {
    // @ts-ignore
    this.robustWeb3 = new RobustWeb3(BridgeConfig.getParam('eth-node-url'))
    ...
    this.tezos = await tezoslib.connect({
    ...
    // Declare Tezos2EthClient contract.
    ...
    try {
      ...
      if (!isInitialized) {
        ...
        // Get most recent block from Tezos blockchain.
        const status = await this.tezos.connection.provider.status()
        // Get the block two blocks before that, to make sure it is final.
        const headBlock = await this.tezos.connection.provider.block({
        ...
        while (!lightClientBlock) {
          // @ts-ignore
          currentValidators = await this.tezos.connection.provider.sendJsonRpc(
          ...
          lightClientBlock = await this.tezos.connection.provider.sendJsonRpc(
  ...
  async runInternal(submitInvalidBlock) {
    ...
    const tezos = this.tezos
    const ethMasterAccount = this.ethMasterAccount
    const web3 = this.web3

    const minDelay = Number(BridgeConfig.getParam('tezos2eth-relay-min-delay'))
    const maxDelay = Number(BridgeConfig.getParam('tezos2eth-relay-max-delay'))
    const errorDelay = Number(BridgeConfig.getParam('tezos2eth-relay-error-delay'))
    ...
    while (true) {
      try {
        ...
        let lastBlock = await tezos.connection.provider.sendJsonRpc('next_light_client_block', [bs58.encode(currentBlockHash)])
  ...
  DANGER_submitInvalidTezosBlock() {
...
exports.Tezos2EthRelay = Tezos2EthRelay
