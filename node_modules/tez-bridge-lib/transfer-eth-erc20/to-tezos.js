...
class TransferETHERC20ToTezos {
  ...
  static async findProof({ extractor, lockedEvent, web3 }) {
    ...
    // header_rlp is again .serialize of the web3.eth.getBlock(block.number)
    //using without .serialize as of now eth-smartpy lib doesnt exist like eth-rust with rlp.encode and rlp.decode to be used in smart contracts.  
    const header_data =  await extractor.extractBlock(block.number)
  ...
  static async waitBlockSafe({
    ...
    //figure out serialize and deserialize for smartpy, and its effects of params being open to world
    //no issue with params being open, as these are proofs of certain log on eth chain which is already open.
    //send data directly, near uses eth-rust module rlp.encode and rlp.decode 
    
    const log_entry_data = log 
    //logFromWeb3(log).serialize()
    
    const receipt_data = receipt 
    //receiptFromWeb3(receipt).serialize()
