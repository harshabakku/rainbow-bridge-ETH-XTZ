...
use eth_types::*;
use tezos_primitives::transaction::ExecutionStatus;
use tezos_sdk::{testing_env, MockedBlockchain};
use utils::{
...
fn setup_factory() -> (RuntimeStandalone, ExternalUser) {
    let (mut r, tezos) = new_root("tezos".into());
    tezos.init_eth_client(&mut r, "eth-client".to_string(), true)
        .unwrap();
    tezos.init_eth_prover(&mut r, "eth-prover".to_string(), "eth-client".to_string())
        .unwrap();
    (r, tezos)
}
...
fn block_hash_safe_from_eth_client() {
    ...
    testing_env!(get_context(vec![], false)); // For use rlp::decode, eth_types::tezos_keccak256
