const { web3BlockToRlp } = require('../eth2tezos-relay')

const Web3 = require('web3')
const BN = require('bn.js')
const { BorshContract, hexToBuffer, readerToHex } = require('../tezbridge/borsh')
...
//call comes from init/tezos-contracts.js 
//responsible for creation of contract or initialization of eth client contract written in smartPy
//being rewritten to etherium client contract deployment on Tezos Chain
//for now contract can also be deployed manually with truffle and take easy way if relayer can directly interact with the deployed contract 
class EthOnTezosClientContract extends BorshContract {
  ...
  async maybeInitialize(validate_ethash, trusted_signer, robustWeb3) {
    ...
    if (!initialized) {
      console.log('EthOnTezosClient is not initialized, initializing...')
      ...
      console.log('EthOnTezosClient initialized')
    }

    console.log('Checking EthOnTezosClient initialization.')
    // @ts-ignore
    ...
    if (
      ...
      console.log(
        `EthOnTezosClient initialization error! The first and last roots are ${first_root} and ${last_root}`
      )
      process.exit(1)
    }
  }
...
exports.EthOnTezosClientContract = EthOnTezosClientContract
