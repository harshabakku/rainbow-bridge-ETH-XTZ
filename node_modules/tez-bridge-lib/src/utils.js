...
// const tezosAPI = require('tezos-api-js')
const Web3 = require('web3')

const CREDENTIALS_DIR = '.tezos-credentials'
const PROJECT_KEY_DIR = './tezosdev'

const DEFAULT_GAS = 1000000

async function setupTezos(config) {
  const deps = await createLocalKeyStore(config.networkId, config.keyPath)
  if (config.keyPath) {
    delete config.keyPath
  }
  return tezosAPI.connect({
    networkId: config.networkId,
    nodeUrl: config.tezosNodeUrl,
    deps,
  })
}
...
/**
 * Setup connection to TEZOS and Ethereum from given configuration.
 * @param {Object} config Config object which defines tezosNodeUrl/ethNodeUrl, networkId and more.
 */
async function setupEthTezos(config) {
  const tezos = await setupTezos(config)
...
async function accountExists(connection, accountId) {
  try {
    const account = new tezosAPI.Account(connection, accountId)
    await account.state()
    return true
  } catch (error) {
...
async function createLocalKeyStore(networkId, keyPath) {
  // TODO: this should live in tezos-api-js
  const credentialsPath = path.join(homedir, CREDENTIALS_DIR)
  const keyStores = [
    new tezosAPI.keyStores.UnencryptedFileSystemKeyStore(credentialsPath),
    new tezosAPI.keyStores.UnencryptedFileSystemKeyStore(PROJECT_KEY_DIR),
  ]
  if (keyPath) {
    const account = JSON.parse(fs.readFileSync(keyPath).toString())
    const keyPair = tezosAPI.utils.KeyPair.fromString(account.secret_key)
    const keyStore = new tezosAPI.keyStores.InMemoryKeyStore()
    keyStore.setKey(networkId, account.account_id, keyPair).then(() => {})
    keyStores.push(keyStore)
  }
  return { keyStore: new tezosAPI.keyStores.MergeKeyStore(keyStores) }
}
