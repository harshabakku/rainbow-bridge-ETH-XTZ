...
async function tezosJsonContractFunctionCall(
  contractId,
  sender,
  method,
  args,
  ...
  // A robust version of tezos-api-js account.functionCall. We can't simply retry account.functionCall because
  ...
  return await signAndSendTransaction(accessKey, sender, contractId, [
    tezoslib.transactions.functionCall(
      method,
  ...
  for (let i = 0; i < RETRY_SEND_TX; i++) {
    try {
      if (resendLast) {
        ...
        let signedTx
        ;[txHash, signedTx] = await tezoslib.transactions.signTransaction(
          receiverId,
          ++accessKey.nonce,
          actions,
          tezoslib.utils.serialize.base_decode(
        ...
        sendTxnAsync = async () => {
          ...
          console.log('TxHash', tezoslib.utils.serialize.base_encode(txHash))
        }
        await sendTxnAsync()
      }
    } catch (e) {
