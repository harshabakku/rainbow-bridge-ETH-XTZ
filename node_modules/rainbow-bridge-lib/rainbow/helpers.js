...
async function maybeCreateAccount(
  ...
  if (!(await accountExists(tezos, accountId))) {
    console.log('Account %s does not exist creating it.', accountId)
    const masterAccount = new tezoslib.Account(tezos.connection, masterAccountId)
    ...
    const account = new tezoslib.Account(tezos.connection, accountId)
...
async function accountExists(tezos, accountId) {
  const account = new tezoslib.Account(tezos.connection, accountId)
  try {
...
async function accountHasTheKey(tezos, accountId) {
  const account = new tezoslib.Account(tezos.connection, accountId)
  const keyStoreKey = await tezos.config.deps.keyStore.getKey(
    tezos.config.networkId,
    accountId
  )
...
async function verifyAccount(tezos, accountId) {
  if (!(await accountExists(tezos, accountId))) {
    console.log(
  ...
  if (!(await accountHasTheKey(tezos, accountId))) {
    console.log(
...
// Used in Status
async function verifyAccountGently(tezos, accountId) {
  if (!(await accountExists(tezos, accountId))) {
    return false
  }
