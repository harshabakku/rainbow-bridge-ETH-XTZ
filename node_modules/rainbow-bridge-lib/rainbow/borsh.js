...
function serialize(schema, fieldType, obj) {
  ...
  const writer = new tezoslib.utils.serialize.BinaryWriter()
...
const signAndSendTransactionAsync = async (
  ...
  let [txHash, signedTx] = await tezoslib.transactions.signTransaction(
    receiverId,
    ++accessKey.nonce,
    actions,
    tezoslib.utils.serialize.base_decode(status.sync_info.latest_block_hash),
        ...
        value: async (args) => {
          ...
          const result = await backoff(10, () =>
            this.account.connection.provider.query(
              ...
              tezoslib.utils.serialize.base_encode(args)
            )
          )
          if (result.logs) {
