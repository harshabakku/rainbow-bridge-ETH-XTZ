...
class Watchdog {
  async initialize() {
    ...
    console.log('Deploying Tezos2EthClient contract.')
  ...
  async run() {
    let privateKey = BridgeConfig.getParam('eth-master-sk')
    if (privateKey.startsWith('0x')) {
      privateKey = privateKey.slice(2)
    }
    privateKey = Buffer.from(privateKey, 'hex')
    const delay = BridgeConfig.getParam('watchdog-delay')
    const errorDelay = BridgeConfig.getParam('watchdog-error-delay')
    while (true) {
      try {
                ...
                while (gasPrice < 10000 * 1e9) {
                  try {
                    ...
                    let tx = new Tx({
                      ...
                      to: BridgeConfig.getParam('eth-client-address'),
