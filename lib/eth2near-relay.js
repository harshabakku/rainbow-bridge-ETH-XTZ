...
const ethClientContractAddress = 'KT1V4AWsawfKWZCwXsxEowyqBtSPtH6HW85n';
...
class Eth2NearRelay {
  ...
  async run() {
    ...
    //  console.log ( "ethClient contract block number "+ new BigNumber(storage.latest_block).toString())
    ...
    while (true) {
      ...
      try {
        ...
        console.log('ETH Chain block number is ' + chainBlockNumber)
        ...
        console.log(ethClientStorage)
        clientBlockNumber = ethClientStorage.latest_block
        console.log('ETH Client block number on Tezos Chain is ' + clientBlockNumber)
  ...
  async submitBlock(block, blockNumber) {
    ...
    // console.log(dfd)
    const txHash= await this.ethClientContract.methods.add_block_header(web3Block.parentHash,web3Block.number,web3Block.hash).send()
    // const txHash = await this.ethClientContract.methods.increment('0xFFFDFD672FF9075528F51A30408CF768A093D8C67FB3C5C8782DFF49EAB0724D').send();
    console.log(txHash)
