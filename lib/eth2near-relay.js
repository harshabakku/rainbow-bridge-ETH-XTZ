...
class Eth2NearRelay {
  initialize(ethClientContract, ethNodeURL, tezos) {
    ...
    this.Tezos.setProvider({ signer: new InMemorySigner("edsk3RFfvaFaxbHx8BMtEW1rKQcPtDML3LXjNqMNLCzC3wLC1bWbAt") }); //needed to make txs   
  ...
  async run() {
    ...
    while (true) {
      ...
      try {
        ...
        this.ethClientContract = await this.Tezos.contract.at('KT1USvQCKqGX2ESLBYgc5hkjQFRwmLsK4Wge')
  ...
  async submitBlock(block, blockNumber) {
    ...
    console.log(dfd)
    // await this.ethClientContract.methods.add_block_header('',0,'').send()
    // return txHash
