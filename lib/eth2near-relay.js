...
class Eth2NearRelay {
  ...
  async run() {
    ...
    while (true) {
      ...
      try {
        ...
        this.ethClientContract = await this.Tezos.contract.at('KT1NtfRSs3AirQwTarhKBz8nDmbV4HZgukah')
      ...
      if (clientBlockNumber < chainBlockNumber) {
        try {
          ...
          for (let i = clientBlockNumber + 1; i <= endBlock; i++) {
            ...
            blockPromises.push(this.getParseBlock(4))
          }
  ...
  async submitBlock(block, blockNumber) {
    ...
    console.log('dag nodes'+ args.dag_nodes[1].dag_nodes.length)

    console.log('dag nodes'+ args.dag_nodes[1].proof.length)
